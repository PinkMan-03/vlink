...
测试套件覆盖 `throughput`、`latency`、`topology`、`serialization` 四类；执行模式支持 `local-direct`、`local-loop`、`process`；拓扑覆盖 `1:1`、`1:n`、`n:1`、`n:n`。完整的命令行参数、终端交互按键、HTML 报告结构和默认测试策略详见 [13-cli-tools.md#vlink-bench](13-cli-tools.md#1311-vlink-bench--发布订阅性能基准测试)。
