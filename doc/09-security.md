...
| 方法                 | 说明                                                        |
| -------------------- | ----------------------------------------------------------- |
| `set_key(key)`       | 设置 AES-128 密钥（必须为 16 字节）；传空字符串恢复内置默认 |
...
内置 AES-128-CBC 默认密钥与 IV 都是 16 字节字面量，**仅用于演示与单元测试**：

| 参数 | 长度    | OpenSSL 实际使用                |
| ---- | ------- | ------------------------------- |
| Key  | 16 字节 | AES-128-CBC 块大小             |
| IV   | 16 字节 | AES-128-CBC 块大小             |

> 内置默认密钥/IV 不应在生产中使用，**部署前务必通过 `set_security_key()` 或自定义 callback 注入业务密钥**。
