...
- **空输入**：`Bytes::empty()` 时 `encrypt()` / `decrypt()` 直接返回 `false` 并清空 `out`（AEAD 不能对零字节做认证，合法密文至少 28 字节 = 12B nonce + 16B tag）。
...
### 示例 2：Protobuf 消息 + passphrase + PBKDF2
