...
容器尾部带 16 位和 32 位用户预留字段，可通过 `get_reserved16()`、`get_reserved32()`、
`get_reserved2()` 访问；`get_reserved()` 是 `get_reserved32()` 的兼容别名。
这些预留字段不被 `clear()` 重置，也不参与同类对象的 `shallow_copy` / `deep_copy` /
`move_copy`，但会随二进制线缆格式序列化和反序列化。
...
| `get_reserved()` / `get_reserved32()` | 主 32 位用户预留字段引用（`get_reserved()` 为兼容别名） |
| `get_reserved16()` / `get_reserved2()` | 16 位 / 第二个 32 位用户预留字段引用                  |
| `clear()`                         | 释放并归零（不含 `reserved16/32/2`）                    |
...
容器尾部带 8 / 16 / 32 位用户预留字段，可通过 `get_reserved8()`、`get_reserved16()`、
`get_reserved32()` 访问；`get_reserved()` 是 `get_reserved32()` 的兼容别名。
这些预留字段不被 `clear()` 重置，也不参与同类对象的 `shallow_copy` / `deep_copy` /
`move_copy`，但会随二进制线缆格式序列化和反序列化。
...
| `get_reserved()` / `get_reserved32()`                  | 主 32 位用户预留字段引用（`get_reserved()` 为兼容别名） |
| `get_reserved8()` / `get_reserved16()`                 | 8 位 / 16 位用户预留字段引用                    |
| `clear()`                                              | 释放并归零（不含 `reserved8/16/32`）             |
...
校验）。
容器尾部带 8 / 16 / 32 位及额外 32 位用户预留字段，可通过 `get_reserved8()`、
`get_reserved16()`、`get_reserved32()`、`get_reserved2()`、`get_reserved3()` 访问；
`get_reserved()` 是 `get_reserved32()` 的兼容别名。这些预留字段不被 `clear()` 重置，
也不参与同类对象的 `shallow_copy` / `deep_copy` / `move_copy`，但会随二进制线缆格式
序列化和反序列化。
...
| `get_reserved()` / `get_reserved32()`    | 主 32 位用户预留字段引用（`get_reserved()` 为兼容别名） |
| `get_reserved8()` / `get_reserved16()`   | 8 位 / 16 位用户预留字段引用                         |
| `get_reserved2()` / `get_reserved3()`    | 第二 / 第三个 32 位用户预留字段引用                   |
| `clear()`                                | 释放并归零（不含 `reserved8/16/32/2/3`）              |
...
容器尾部带 8 位和 32 位用户预留字段，可通过 `get_reserved8()`、`get_reserved32()`
访问；`get_reserved()` 是 `get_reserved32()` 的兼容别名。这些预留字段不被
`clear()` 重置，也不参与同类对象的 `shallow_copy` / `deep_copy` / `move_copy`，
但会随二进制线缆格式序列化和反序列化。
...
| `get_reserved()` / `get_reserved32()`    | 主 32 位用户预留字段引用（`get_reserved()` 为兼容别名） |
| `get_reserved8()`                        | 8 位用户预留字段引用                                   |
| `clear()`                                | 释放并归零（不含 `reserved8/32`）                      |
