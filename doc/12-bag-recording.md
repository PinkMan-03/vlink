...
**各后端的实际行为**（源码参见 `vdb_writer.cc:184`、`vcap_writer.cc:163`）：
...
// 后缀不支持时返回 nullptr。最后一个引用释放时自动从全局表注销。
...
// 如果绑定了 BagReaderPluginInterface，filter_urls 匹配 convert_url_meta() 后的回放 URL。
