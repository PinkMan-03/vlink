...
| `--proxy_max_packet_size` | `proxy_server` 模式下允许转发的最大 payload（MiB）。**webviz 的 proxy_bridge 中 `0.0` 表示不限制**（`webviz/proxy_bridge.cc:245,917` 仅对 `>0` 启用过滤）；**注意与 `vlink-proxy` 同名字段语义相反**（见 [代理 -- 注意 0=丢全部](16-proxy.md#proxyserver-config)），后者 0 会丢弃所有非空消息 | `0.0` |
