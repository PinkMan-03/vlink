...
- `proxy_api`：WebViz 作为外部客户端，通过 `ProxyAPI` 连接独立运行的 `ProxyServer`；默认先完成 Handshake RPC 获取 token，后续 Control 带 token，Time 心跳回显 token
...
- `proxy_api` 适合复用独立代理的场景，`--proxy_reliable`、`--proxy_direct`、`--proxy_no_match_version` 只在该模式下生效；默认握手由 `ProxyAPI` 自动完成，token 失配会通过 `kTokenError` 上报并触发重握手
