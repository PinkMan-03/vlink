...
> **相关文档**：可视化工具 vlink-viewer 使用 ProxyAPI 连接参见 [14-viewer.md](14-viewer.md)；CLI 监控工具参见 [13-cli-tools.md](13-cli-tools.md#vlink-monitor--实时通信状态监控)；服务发现机制参见 [17-discovery.md](17-discovery.md)。
...
| `security_key`         | `std::string`           | `""`     | Time/InfoList/Control 通道的对称密钥。**必须非空**——这三条通道使用 `SecurityPublisher`/`SecuritySubscriber`，运行时不再有公共 `enable_security()` 入口，`Security::Config::key` 只能通过构造函数传入。空字符串会让 `security_` 保持空，控制面消息被 drop（启动时打印 `VLOG_W`）。客户端与服务端的 `security_key` 必须一致。 |
...
- 启动时环境变量 `VLINK_INTRA_BIND`（参见 [21-environment-vars.md](21-environment-vars.md#核心运行时环境变量)）若被设置，服务器将同时订阅 `intra://` 话题
...
`vlink-monitor`（参见 [13-cli-tools.md](13-cli-tools.md#vlink-monitor--实时通信状态监控)）是 VLink 提供的命令行监控工具，内部使用 `DiscoveryViewer` 发现节点和话题。`vlink-viewer`（参见 [14-viewer.md](14-viewer.md)）则通过 `ProxyAPI` 连接到 `vlink-proxy` 获取详细数据。
