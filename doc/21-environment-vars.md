...
所有 VLink 专有环境变量以 `VLINK_` 前缀开头。`vlink-check env` 会按编译时启用的模块（`VLINK_SUPPORT_*` 宏）和 CLI 子工具（`VLINK_ENABLE_CLI_*`）动态裁剪输出：模块未编译则对应私有 env 不会展示（例如未启用 Zenoh 时不列 `VLINK_ZENOH_*`、未启用 SHM2 时不列 `VLINK_SHM2_*`）。若要确认某变量是否被使用，最权威的方法是搜索源码 `Utils::get_env("<NAME>")`。参见 [13-cli-tools.md](13-cli-tools.md#vlink-check--系统配置与环境诊断)。

---

## 目录

1. [核心运行时环境变量](#核心运行时环境变量)
2. [日志控制环境变量](#日志控制环境变量)
3. [发现与诊断环境变量](#发现与诊断环境变量)
4. [DDS 传输环境变量](#dds-传输环境变量)
5. [SHM 传输环境变量](#shm-传输环境变量)
6. [其他传输后端环境变量](#其他传输后端环境变量)
7. [Bag 录制环境变量](#bag-录制环境变量)
8. [Bench 工具环境变量](#bench-工具环境变量)
9. [外部依赖环境变量](#外部依赖环境变量)
10. [交叉编译专用环境变量](#交叉编译专用环境变量)
11. [示例应用中的自定义环境变量](#示例应用中的自定义环境变量)
12. [设置方式](#设置方式)
...
| `VLINK_MEMORY_LEVEL`    | 数字     | `vlink::MemoryPool` 默认 8 阶金字塔的分级（`0`..`9`，默认 `3` Balanced）。`0` = bypass 模式：跳过所有 tier，每次 `allocate` 直接 `::operator new`、每次 `deallocate` 直接 `::operator delete`。`1..9` 选择内置金字塔，等级越高每档预留 block 越多；`9` 满载占用上限不超过 500 MiB。非数字或越界值钳到 `[0, 9]` 并打 warning。仅在首次构造全局池时读取，由 `Bytes::init_memory_pool()` / `MemoryPool::global_instance(true)` 触发。 |
...
录制/回放 API 详细说明参见 [12-bag-recording.md](12-bag-recording.md)；CLI 工具参见 [13-cli-tools.md](13-cli-tools.md#vlink-bag--数据录制与回放)。
