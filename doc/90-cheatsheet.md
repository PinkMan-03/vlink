...
1. [30 秒骨架](#30-秒骨架)
2. [六种原语 API](#六种原语-api)
3. [Node 基类方法](#node-基类方法)
4. [核心枚举值](#核心枚举值)
5. [Transport 与 URL](#transport-与-url)
6. [QoS 完整结构](#qos-完整结构)
7. [QoS 预设](#qos-预设qosxxx)
8. [序列化推导](#序列化t-会走哪种-kxtype)
9. [安全 API](#安全-api)
10. [零拷贝 API](#零拷贝-api)
11. [日志宏](#日志宏)
12. [Bag 录制/回放](#bag-录制回放)
13. [Discovery / Proxy](#discovery--proxy)
14. [C API](#c-api)
15. [9 个 CLI 工具](#9-个-cli-工具)
16. [CMake 选项](#cmake-选项)
17. [VLINK_* 公共宏](#vlink_-公共宏)
18. [环境变量全表](#环境变量全表前缀-vlink_)
19. [常用代码模式](#常用代码模式)
20. [快速排障](#快速排障)
21. [examples/ 目录](#examples-目录结构)
...
```cpp
vlink::SecurityPublisher<Msg> pub("dds://topic");
pub.set_security_key(/*≥16 B key*/);            // 内置 AES-128-CBC (+PKCS7)
// 或：
pub.set_security_callbacks(enc_cb, dec_cb);     // 自定义算法
```
...
```cpp
if (pub.is_support_loan()) {                   // 仅 shm/shm2/intra 返回 true
...
### DiscoveryReporter / DiscoveryViewer
...
### ProxyServer / ProxyAPI / vlink-proxy
...
> 完整描述与按模块宏裁剪逻辑见 [21-environment-vars.md](21-environment-vars.md)。`vlink-check env` 按编译时启用的模块（`VLINK_SUPPORT_*` / `VLINK_ENABLE_CLI_BENCH`）动态裁剪输出。
