...
**源码**：`src/extension/bag_reader_processor.cc:98`。
...
- **`-x/--max_packet_size` 设成 `0` 后所有非空消息被丢弃** → 过滤逻辑 `if (bytes.size() > real_max_packet_size) return;`（`proxy_server.cc:1313`）没有 `0 = unlimited` 的特判；CLI 默认是 `4.0` MiB，若需放行大包请显式设置足够大的 MiB 值（例如 `-x 64`）。头文件注释中写的 "0 = unlimited" 与当前实现不符
