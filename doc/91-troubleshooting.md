...
**源码**：`src/extension/vdb_reader.cc:1714`。
...
**源码**：`src/extension/vcap_reader.cc:842`。
...
**源码**：`src/extension/vdb_writer.cc:1272-1277`。
...
**源码**：`src/extension/vdb_writer.cc:317`、`vdb_reader.cc:155`。
...
| `VCAPWriter: Writer is not open.` | `vcap_writer.cc:675` | bag writer 还没 init |
...
| 错误 | 源 | 可做的 |
|---|---|---|
| `VDBReader: Database version is incompatible.` | `vdb_reader.cc:1714` | 换回录制端相同版本 |
| `VDBReader: Incomplete data detected.` | `vdb_reader.cc:348` | `vlink-bag fix` 修 |
| `VDBReader: Invalid start_timestamp.` | `vdb_reader.cc:1743` | `-b` 超出 bag 时间段 |
| `VDBReader: Database accuracy is not supported.` | `vdb_reader.cc:1756` | VDB 精度版本差太多 |
| `VCAPReader: Mcap version is incompatible.` | `vcap_reader.cc:842` | 升级 VLink |
| `VCAPReader: DB list is empty.` | `vcap_reader.cc:1314` | MCAP 头损坏 |
| `VCAPReader: JSON parse error, ...` | `vcap_reader.cc:1430` | metadata JSON 坏 |
