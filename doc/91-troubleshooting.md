...
>
> 万能第一步：`vlink-check diag`。它会一次性跑 40+ 项自检（见 [§1](#1-先跑-vlink-check-diag)）。
>
> **找不到你的问题？** 直接跳到 [§24 按错误字符串速查](#24-按错误字符串速查reverse-lookup)。

目录：

1. [先跑 vlink-check diag](#1-先跑-vlink-check-diag)
2. [构建期问题](#2-构建期问题)
3. [启动期问题](#3-启动期问题)
4. [通信建立不起来 / 收不到数据](#4-通信建立不起来--收不到数据)
5. [性能问题（延迟抖、丢样本、CPU 高）](#5-性能问题延迟抖丢样本cpu-高)
6. [SHM / Iceoryx 专属](#6-shm--iceoryx-专属)
7. [DDS 专属](#7-dds-专属)
8. [Bag 录制回放](#8-bag-录制回放)
9. [C API / Python / 跨语言](#9-c-api--python--跨语言)
10. [CLI 工具异常](#10-cli-工具异常)
11. [还没排出来？](#11-还没排出来)
12. [每个 CLI 工具错误目录](#12-每个-cli-工具错误目录)
13. [每个 Transport 专属 init 错误](#13-每个-transport-专属-init-错误)
14. [运行时异常分类速查](#14-运行时异常分类速查)
15. [Discovery 深入](#15-discovery-深入)
16. [平台专属陷阱](#16-平台专属陷阱)
17. [QoS + 安全兼容性](#17-qos--安全兼容性)
18. [容器 / K8s / VPN 部署](#18-容器--k8s--vpn-部署)
19. [插件加载失败](#19-插件加载失败)
20. [Bag 损坏 / 恢复](#20-bag-损坏--恢复)
21. [日志调优](#21-日志调优)
22. [版本 / ABI 不匹配](#22-版本--abi-不匹配)
23. [性能反模式 & 调试技巧](#23-性能反模式--调试技巧)
24. [按错误字符串速查（Reverse Lookup）](#24-按错误字符串速查reverse-lookup)

---

## 1. 先跑 `vlink-check diag`
...
### 2.1 编译时 `static_assert: is_supported(type)` / `kUnknownType`

**原因**：你给 `Publisher<T>` / `Subscriber<T>` 等传了一个 `Serializer` 不认识的类型。

**如何确认**：看看 `T` 是否满足 [06-serialization.md](06-serialization.md#自动类型推断机制) 14 种触发条件中的某一个。
...
### 2.2 `Could not find package vlink::<module>`
...
### 2.3 找不到 `iceoryx_posh` / `fastdds` / 其它 third-party
...
### 2.5 `-Wconversion` / `-Werror` 开着的时候 VLink 头报一堆 warning
...
### 2.6 `clang-tidy` 报一堆 `readability-identifier-naming` 在 VLink 头
...
### 2.7 `undefined reference to vlink::...`
...
### 3.1 构造 Publisher 直接抛异常（`vlink::Exception::RuntimeError`）

**常见根因**（源：`src/impl/url_parser.cc`）：
- URL 格式错误 —— 具体错误消息见 [§19](#19-插件加载失败) 和 [§24 速查](#24-按错误字符串速查reverse-lookup)：
...
### 3.2 进程一启动就挂死（没日志、没报错）

**常见根因**：
- `shm://` 但 SHM 守护进程没起（见 [§6](#6-shm--iceoryx-专属)）
...
**修复**：检查文件存在；`ldd plugin.so` 看依赖；插件符号必须用 `VLINK_PLUGIN_EXPORT` 宏导出。完整插件错误见 [§19](#19-插件加载失败)。

### 3.4 `Program has started.`（vlink-proxy 拒绝启动）
...
### 4.1 `pub.has_subscribers()` 永远为 `false`
...
### 4.4 `SecurityPublisher` 与 `SecuritySubscriber` 连通但乱码

**原因**：密钥不一致；或一端设了 `set_security_key` 另一端设了 `set_security_callbacks`。

**修复**：两端用完全相同的配置。并检查 OpenSSL 版本是否匹配（不同 OpenSSL 的 PKCS7 填充细节不完全相同，但 VLink 统一用 EVP API，一般没事）。

### 4.5 `Server::listen` 被调用了两次
...
### 6.5 `ShmConf: Input string length is too long.`
...
### 7.2 Multiple nodes with same DDS participant name

**原因**：VLink 为每个进程起一个 DDS Participant，但某些场景（同进程起多个 Publisher/Subscriber）共享同一个 Participant。正常。

### 7.3 DDS Security（RTPS SECURE）与 VLink Security
...
### 7.5 `Topic ... has no registered typesupport.`
...
### 7.6 `ssl.* properties are set but CycloneDDS was built without DDS_HAS_SSL support.`
...
### 7.7 RTI Connext（ddsr://）报 `DDS_DomainParticipantFactory_create_participant failed.`
...
### 8.1 `.vdb` 打不开 / 损坏（`Database version is incompatible.`）
...
### 8.2 MCAP 文件打不开（`Mcap version is incompatible.`）
...
### 8.4 `split_by_size` 和 `split_by_time` 能同时使用吗

可以；任一条件触发都会切。时间单位是**毫秒**。

### 8.5 VCAP（`.vcap`）与 MCAP 标准兼容吗

**是** —— `.vcap` 内部就是 MCAP。可以用 Foxglove 直接打开，也可以 `vlink-bag2mcap` 转一步（主要是改扩展名和补 schema 注册）。

### 8.6 `Cache size is full, waiting to consume.` 刷屏
...
### 8.8 `The compile macro VLINK_ENABLE_SQLITE is not turned on.`
...
### 9.1 `VLINK_RET_RUNTIME_ERROR` 是什么意思
...
### 10.2 `vlink-eproto sub` 报 `Must set proto dir`
...
| `Failed to load plugin (...)` | 1255 | schema 插件缺失（见 [§19](#19-插件加载失败)） |
| `filesystem_error` 回溯 | 多处 | 权限 / 磁盘满 / 路径不存在 |

### 12.2 `vlink-check`（`cli/check/check.cc`）

参见 [§1 的自检表](#1-先跑-vlink-check-diag) + 每项 FAILED 原因：
...
### 12.3 `vlink-dump`（`cli/dump/dump.cc`）
...
### 12.4 `vlink-eproto` / `vlink-efbs`（`cli/eproto/eproto.cc`, `efbs/efbs.cc`）
...
- `Cdr type does not support security.` — 所有 6 种原语都会打（见 [§17](#17-qos--安全兼容性)）
...
| 错误 | 位置 | 诊断 |
|---|---|---|
| `Failed to loan buffer, size: ...` | `shm_factory.cc:640+` | SHM pool 耗尽（见 [§6.2](#shm-loan-failed)） |
...
- `Cdr type does not support security.` — 12 处都会打，见 [§17](#17-qos--安全兼容性)
...
### 15.1 `Failed to set multicast interface to loopback.` (`discovery_reporter.cc:202`)
...
### 15.2 `Failed to set multicast, please add address [...] to target device...` (`discovery_viewer.cc:560-570`)

- 路由表没有 239.255.0.100 的出口
- 修复：`sudo ip route add 239.255.0.100/32 dev eth0`（持久化改 `/etc/network/interfaces` 或 NetworkManager）

### 15.3 `Global discovery reporter is disabled.` (`discovery_reporter.cc:122`)
...
### 15.4 `Url is too long [...]`  /  `Ser type is too long [...]`
...
### 15.5 `DiscoveryViewer` 拿不到 `cpu_usage`
...
- 有专用 `qnx://` transport（见 [§13.11](#1311-qnx)）
...
### 17.2 QoS 预设 vs 自定义 Qos

- 传入 URL 的 `?qos=xxx` 要小写（`kSensor` → `sensor`）
- 自定义 Qos 必须 `valid = true` 才生效
- `KeepAll` 的 depth 参数是**忽略的**（只对 KeepLast 有效）

### 17.3 `DdscFactory: ssl.* properties are set but CycloneDDS was built without DDS_HAS_SSL support.`
...
## 24. 按错误字符串速查（Reverse Lookup）

| 遇到的字符串 | 去哪看 | 一句话 |
|---|---|---|
| `The URL transport prefix cannot be empty.` | [§3.1](#31-构造-publisher-直接抛异常vlinkexceptionruntimeerror) | URL 不含 `xxx://` |
| `Bad key in the query string!` | [§3.1](#31-构造-publisher-直接抛异常vlinkexceptionruntimeerror) | URL `?` 后面的 key 打错 |
| `Unsupported plugin module, libname: ...` | [§3.3](#33-failed-to-load-plugin--unsupported-plugin-module) / [§19](#19-插件加载失败) | transport 没编进或 plugin 不存在 |
| `Cdr type does not support security.` | [§17.1](#171-cdr--security-全面不兼容) | CDR + kWithSecurity 不兼容 |
| `Topic ... has no registered typesupport.` | [§13.3](#133-dds-fast-dds) | DDS schema 没注册 |
| `Failed to loan buffer, size: ...` | [§6.2](#shm-loan-failed) | SHM mempool 不够大 |
| `Shm roudi is not supported.` | [§6.3](#63-shm-roudi-is-not-supported-错误) | RouDi 模式错 |
| `Database version is incompatible.` | [§8.1](#81-vdb-打不开--损坏database-version-is-incompatible) | bag 版本错位 |
| `Mcap version is incompatible.` | [§8.2](#82-mcap-文件打不开mcap-version-is-incompatible) | MCAP 版本错位 |
| `Must set proto dir [-d], ...` | [§10.2](#102-vlink-eproto-sub-报-must-set-proto-dir) | 动态 proto 没 schema 目录 |
| `Plugin: Version mismatch: local X.Y, required X.Z.` | [§22.1](#221-plugin-版本) | plugin ABI 不对 |
| `Cache size is full, waiting to consume.` | [§8.6](#86-cache-size-is-full-waiting-to-consume-刷屏) | 回放消费跟不上 |
| `DiscoveryReporter: Failed to set multicast ...` | [§15.1](#151-failed-to-set-multicast-interface-to-loopback-discovery_reportercc202) | 多播接口配置问题 |
| `Invalid domain id.` | [§12.6](#126-vlink-proxyproxyproxycc) | `vlink-proxy -d` 参数超出 0–255 |
| `VLINK_RET_RUNTIME_ERROR` | [§9.1](#91-vlink_ret_runtime_error-是什么意思) | C++ 构造时抛异常 |
| `VLINK_RET_MEMORY_ERROR` | [§9.3](#93-vlink_ret_memory_error-是什么) | 调用方 buffer 太小 |
| `VLINK_RET_TRANSFER_ERROR` | [§9.4](#94-vlink_ret_transfer_error) | send/recv/invoke 失败 |
| `write_data() type mismatch, expected raw bytes but received ptr type.` | [§14.1](#141-publisher-热路径) | 用错类型发 DDS |
