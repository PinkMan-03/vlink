...
本示例集共包含 **14 个分类目录**、**87 个独立示例工程**，其中 85 个默认参与构建（`samples/dds_idl` 在父级 `examples/samples/CMakeLists.txt:21` 注释禁用，`serialization/intra_data` 在父级 `examples/serialization/CMakeLists.txt:25` 注释禁用，需手动取消注释才会被加入构建）。
...
### security/ -- 加密与认证（3 个工程）

通信安全功能教程。VLink 提供 Security 前缀的通信原语变体（SecurityPublisher、SecurityClient 等），通过 AES 对称加密保护通信内容。

| 工程 | 说明 |
|------|------|
| `security_basic` | Security 变体基本用法 -- SecurityPublisher/SecuritySubscriber 等 |
| `security_custom` | 自定义加密回调 -- set_security_callbacks() 安装 encrypt/decrypt 回调，绕过内置 AES |
| `security_ssl` | SSL/TLS 安全传输 |

### zerocopy/ -- 零拷贝传输（4 个工程）
...
**实践建议**：`samples/shm_raw/` 已完整演示了 Security 变体的六种通信原语，包括 `set_security_key()` 自定义密钥和 `SecurityServer/Client/Publisher/Subscriber/Setter/Getter` 的实际用法，是学习安全通信的最佳起点。
...
每种原语都有对应的 Security 变体（如 `SecurityPublisher<T>`），通过 `set_security_key()` 设置对称加密密钥。
...
| 分类目录总数 | 14 |
| 示例工程总数 | 87 |
| 已实现工程数 | 85（`samples/dds_idl` 在 `samples/CMakeLists.txt:21` 注释禁用，需手动启用 FastDDS IDL 工具链；`serialization/intra_data` 在 `serialization/CMakeLists.txt:25` 注释禁用，需启用 Protobuf + intra:// 后端） |
| 源文件数量 | 98 个 .cc + 3 个 .c + 33 个 .h + 4 个 .proto + 2 个 .fbs + 1 个 .idl |
