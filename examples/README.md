...
通信安全功能教程。VLink 提供 Security 前缀的通信原语变体（SecurityPublisher、SecurityClient 等），在构造时通过第二参数 `const Security::Config&` 一次性传入对称密钥、RSA 密钥或自定义回调，保护通信内容。
...
每种原语都有对应的 Security 变体（如 `SecurityPublisher<T>`）。先构造 `Security::Config`（例如 `cfg.key = "..."`），然后作为 `SecurityXxx` 构造函数的第二参数传入即可，例如 `SecurityPublisher<T> pub(url, cfg)` 或 `SecurityPublisher<T> pub(url, cfg, InitType::kWithoutInit)`。运行时不再有公共 `enable_security()` / `security()` 接口。
