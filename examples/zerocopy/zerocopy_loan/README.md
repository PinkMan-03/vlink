...
本示例演示 VLink 的零拷贝 Loan API，包括 `loan()`、`return_loan()`、`is_support_loan()` 和 `set_manual_unloan()`。Loan API 允许发布者从传输层的内存池中借用��冲区，直接填充数据后发布，避免额外的内存分配和拷贝。
...
| `set_manual_unloan(true)` | 订阅者手动控制缓冲区生命周�� |
