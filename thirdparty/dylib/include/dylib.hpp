...
class dylib {
 public:
  ...
  dylib &operator=(dylib &&other) noexcept {
    if (this != &other) std::swap(m_handle, other.m_handle);
    return *this;
  }
  ...
  dylib(const char *dir_path, const char *lib_name, bool decorations = add_filename_decorations) {
    if (!dir_path) throw std::invalid_argument("The directory path is null");
    if (!lib_name) throw std::invalid_argument("The library name is null");

    std::string final_name = lib_name;
    std::string final_path = dir_path;

    if (decorations) final_name = filename_components::prefix + final_name + filename_components::suffix;
  ...
  native_symbol_type get_symbol(const char *symbol_name) const {
    ...
    if (!m_handle) throw std::logic_error("The dynamic library handle is null. This object may have been moved from.");
...
#pragma GCC diagnostic pop
  ...
  bool has_symbol(const char *symbol_name) const noexcept {
    if (!m_handle || !symbol_name) return false;
 ...
 protected:
  ...
  static void close(native_handle_type lib) noexcept { DYLIB_WIN_OTHER(FreeLibrary, dlclose)(lib); }
